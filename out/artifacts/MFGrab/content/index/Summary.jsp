...
<div class="admin-content">
    <div class="admin-content-body">
        <div class="am-cf am-padding am-padding-bottom-0">
            <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">详细记录</strong>
            </div>
        </div>
        ...
        <div class="am-g">
            <div class="am-u-sm-12 am-u-md-4">
                <div class="am-form-group">
                    请选择渠道：
                    <select id="selectedChannel" data-am-selected="{btnWidth:'50%',btnSize:'sm',maxHeight:250}">
                        <option value="464"
                                <c:if test="${requestScope.pageModel.channel == 464}">selected</c:if> >464&nbsp;&nbsp;yiyuan
                        </option>
                        <option value="625"
                                <c:if test="${requestScope.pageModel.channel == 625}">selected</c:if> >625&nbsp;&nbsp;oppo
                        </option>
                        <option value="626"
                                <c:if test="${requestScope.pageModel.channel == 626}">selected</c:if> >626&nbsp;&nbsp;vivo
                        </option>
                        <option value="627"
                                <c:if test="${requestScope.pageModel.channel == 627}">selected</c:if> >627&nbsp;&nbsp;huawei
                        </option>
                        <option value="628"
                                <c:if test="${requestScope.pageModel.channel == 628}">selected</c:if> >628&nbsp;&nbsp;xiaomi
                        </option>
                        <option value="629"
                                <c:if test="${requestScope.pageModel.channel == 629}">selected</c:if> >629&nbsp;&nbsp;qq
                        </option>
                    </select>
                </div>
            </div>
                ...
                <form class="am-form">
                    <table class="am-table am-table-striped am-table-hover table-main">
                        ...
                        <tbody id="content_table">
                            <c:forEach items="${SummaryList}" var="rs" varStatus="stat">
                                <tr>
                                    ...
                                    <td>${rs.newcount}</td>
                                    <td>${rs.dayreten}</td>
                                    <td class="am-hide-sm-only"><fmt:formatDate value="${rs.date_origin}"
...
<script>
    ...
    $(function () {
        ...
        function queryWithChannelAndDate() {
            window.location.href = "<c:url value="/Summary/List?"/>"+"channel="+$("#selectedChannel").val()
                                    +"&date_1="+$('#date_1').val()+"&date_2="+$('#date_2').val()
        }
